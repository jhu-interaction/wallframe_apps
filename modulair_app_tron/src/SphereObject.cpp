...
//{
//  if(_sph){
//    _sph->setRadius(radius);
//  }
//}

double SphereObject::getRadius() const
{
  if(_sph){
    return _sph->getRadius();
  }else{
    return -1;
  }
}
...
void SphereObject::setColor(const Vec4 color)
{
  _material->setDiffuse(Material::FRONT, color);
  _material->setEmission(Material::FRONT, color);
  _material->setAlpha(Material::FRONT_AND_BACK,1.0);
  _sphGeode->getOrCreateStateSet()->setAttribute(_material);
}
}